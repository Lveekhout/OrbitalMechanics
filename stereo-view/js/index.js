    ...
    ctx.scale(camera.scale, camera.scale)
    {
        ...
        } // Points
        {
            ...
            for (let idx = 0; idx < lines.length; idx++) {
                const depth = 7
                const d = [lines[idx].from.z / depth + 1, lines[idx].to.z / depth + 1] // f(x) -> 1/(x/2.4+1)
                ctx.moveTo(lines[idx].from.x / d[0]+3.5, -lines[idx].from.y / d[0])
                ctx.lineTo(lines[idx].to.x / d[1]+3.5, -lines[idx].to.y / d[1])
            }
            ctx.stroke()
            ctx.restore()
        } // Points
        ...
        } // Masses
        {
            ...
            for (let idx = 0; idx < masses_.length; idx++) {
                ...
                const d = masses_[idx].z / depth + 1 // f(x) -> 1/(x/2.4+1)
                try {
                    ctx.arc(masses_[idx].x / d+3.5, -masses_[idx].y / d, .1 / d, 0, Math.PI * 2)
                } catch(e) {}
            }
            ctx.fill()
            ctx.restore()
