function Vector() {
    if (arguments.length === 1 && arguments[0].constructor.name === 'Array') {
        ...
        if (arguments[0] < 0) {
            this.length = -arguments[0]
            this.angle = arguments[1] + Math.PI
        } else {
            this.length = arguments[0]
            this.angle = arguments[1]
        }
        ...
        if (arguments[1] < 0) {
            ...
            this.angle = arguments[2] + Math.PI
        } else {
            this.length = arguments[1]
    ...
    this.addPolar = (r = 1, a = 0) => {
        if (r.constructor.name === 'Number' && a.constructor.name === 'Number') {
            this.p[0] += r * Math.cos(a)
            this.p[1] += r * Math.sin(a)
            this.length = Math.sqrt(Math.pow(this.p[0], 2) + Math.pow(this.p[1], 2))
            this.angle = Math.atan2(this.p[1], this.p[0])
        } else throw `addPolar: geen overload voor: (${Array.from(arguments).map(e => e.constructor.name).join(', ')})`
    }
    ...
    this.addVector = v => {
        if (v.constructor.name === 'Vector') {
            ...
            this.length = Math.sqrt(Math.pow(this.p[0], 2) + Math.pow(this.p[1], 2))
            this.angle = Math.atan2(this.p[1], this.p[0])
        } else throw `addVector: geen overload voor: (${Array.from(arguments).map(e => e.constructor.name).join(', ')})`
    }

    this.makeCopy = () => new Vector(this.o, this.p)
