...
function Vector() {
    if (arguments.length === 1 && arguments[0].constructor.name === 'Array') {
        ...
        argArray.forEach((v, i) => {
            arglist.push(v.constructor.name)
        })
        throw `Vector: geen overload voor: (${arglist.join(', ')})`
    }
    // if (this.angle < 0) this.angle += Math.PI * 2

    this.draw = (ctx, point, color, scale = 1) => {
        point = point / camera.scale
        ctx.save()
